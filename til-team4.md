...
```
객체와 배열에 대해서 자세히 들여다보고, 다양한 메소드들을 보면서
많은 기능들을 제공한다는 느낌을 받았고, 알고리즘 문제를 풀어보면서
배운 것들을 써먹어보면서 기능의 역할을 깨닫게되서 좋았습니다.

스크립트로 어떻게 데이터를 정리하고 관리를 할까 생가가했는데.. 다양한 매소드를 보면서
실업에서 어떻식으로 사용할지 감이 와서 좋왔습니다.


지난 시간에 배운 객체의 점표기법과 대괄호 표기법에 대해서 좀 더 잘 알 수 있었습니다. 
또 배열 이론을 배우고 문제에 적용해 보니 배열의 이론에 대해 좀더 잘 알 수 있었습니다.

문자열 을 배열로, 배열을 문자열로 변환하는 방법에 익숙해지기. 빈 배열은 `undefined` 상태.  `splice` 매소드에 대해 이해할 필요가 있음.
